...
Wonderland 0.11.1 后端配套的终端客户端，当前 npm 客户端版本为 0.11.1。Node.js 18+，连接已启动的 Wonderland Rust 后端；后端来自桌面安装包、便携包或源码构建。npm 包不包含后端。预览版本使用 `next` 频道，稳定频道 `latest` 仍为 0.7.0。
...
0.11.0 起支持在 `work create` 时使用全局 `--reasoning`，保存为任务的 `reasoning_effort`；例如 `wonderland-cli --model claude-sonnet-4-6 --reasoning high work create claude "检查错误处理"`。启动使用已保存配置，不接受临时覆盖；省略档位表示沿用官方默认。Teams 同样在执行器绑定中指定 `reasoning_effort`。当前客户端与 0.11.1 后端配套；0.10.0 仍只支持在 Teams 中指定档位。Kimi 当前没有受管 effort 选项。

后端首次打开旧工作流数据库会事务迁移到 schema v3。v2→v3 增加一次性计划与触发审计表；升级前停止服务并备份完整 `AGENT_DATA_DIR`。旧程序不能读取 v3 数据库，回退时使用升级前备份；客户端包本身不保存或迁移这些数据。
