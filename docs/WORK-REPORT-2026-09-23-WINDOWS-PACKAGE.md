...
| ZIP 文件清单与 SHA-256 | 通过；清单符合预期，校验值见本报告 |
...
3. **发布远端核验**：源码已通过 `git push origin main` 推送至 GitHub `69f466a`。npm `0.11.1` 已存在时保持该版本不可重复发布，后续改版再递增版本并发布；若要把本地安装器作为 GitHub Release 资产上传，先核验 tag、资产版本和远端 CI，再创建对应 release。
