...
本说明对应 **0.11.2** 版本安装包（安装与数据目录可选、启动有状态提示、桌面端可检查客户端更新）。

---

## 1. 下载与安装

**下载**：打开 [GitHub 发布页](https://github.com/Alice-Marx/Rust_for_AI-agent/releases/tag/v0.11.2)，下载 `Wonderland-Setup-0.11.2-x64.exe`（也可用免安装的 `Wonderland-0.11.2-windows-x64.zip`，解压后直接运行里面的 `Start-Wonderland.ps1`，然后跳到第 3 节）。
...
> 校验（可选）：下载发布页同一版本的 `SHA256SUMS.txt`，用其中 `Wonderland-Setup-0.11.2-x64.exe` 对应的 SHA-256 核对安装器。
...
客户端升级也在**「应用」**页顶部的「Wonderland 客户端」卡片中完成：先点**「检查更新」**，有 stable Release 时再点**「下载并校验」**，最后点**「安装并重启」**。安装器只会覆盖当前程序目录，保留已选的数据目录；更新前请先保存正在编辑的内容，并停止本窗口中的终端或 API 对话。
...
装好后回到桌面顶部的**「应用」**页：每个工具一张卡片，点**「检测安装」**确认识别成功（只检查安装与版本，不会调用模型、不花钱）。支持的未安装工具会显示**「一键安装（命令明文）」**；它使用卡片显示的固定官方命令，完成后自动重新检测版本。也可继续使用上表命令手动安装。有问题时展开「诊断详情」或「安装输出」看原因。
...
- 版本与验证状态详见 [0.11.2 发布说明](RELEASE-NOTES-0.11.2.md) 与 [总项目任务报告](PROJECT-TASK-REPORT-2026-09-23.md)；
