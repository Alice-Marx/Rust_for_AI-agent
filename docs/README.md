...
- **跨厂商分工实验记录：[EXPERIMENT-2026-09-24](EXPERIMENT-2026-09-24-CROSS-VENDOR-TEAM.md)**——Kimi×DeepSeek 真实分工全流程（一键安装→团队创建→权限批准→独立验收），含 8 轮真实缺陷修复表与重做路径。
- **图文使用说明：[USER-GUIDE](USER-GUIDE.md)**——面向第一次使用 Wonderland 的用户：下载安装（含自定义目录）、启动状态解读、官方工具安装登录、第一个 Work 任务、Teams 多任务协作、终端与常见问题，全部配界面截图。
