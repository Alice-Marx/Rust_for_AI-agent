...
- 新增 `POST /api/v1/workflows/{id}/duplicate`，Rust CLI 对应 `work duplicate <id>`。来源必须是已终止的独立任务；执行中、等待输入、待验收、草稿和团队拥有的 child 都不能通过该操作重试。
...
2. 通过 HTTP 真实路由与 CLI 端到端冒烟检查 `work duplicate` 响应为 Draft，且执行只在用户随后明确调用 `work start` 时开始。当前只验证了 CLI help，未发送真实服务请求。
