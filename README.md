...
**按模型质量和成本自动选择执行器尚未启用。** 每轮条件检查重新获取 LiveBench 与官方价格，当前 Automatic 刷新后仍转为 Blocked；模型身份、计费渠道和订阅额度尚待核验。新增的 `POST /api/v1/teams/{id}/routing/preview` 只做在线证据绑定和可解释候选预览，不启动模型，也不解除该阻塞。预算账本已有实现，但带 USD 上限的原生任务当前不能执行。Kimi Node、MiniMax、MiMo 和 ZCode 当前通过交互终端使用；插件市场、定时任务、远程主机、PR 与网站面板仍属于后续范围。详见 [0.11.0 使用说明与边界](docs/RELEASE-0.11.0.md)。
